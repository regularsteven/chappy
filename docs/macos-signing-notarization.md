...
- `MACOS_CERTIFICATE_NAME` (optional): certificate common name, for example `Developer ID Application: Your Name (TEAMID)` (workflow auto-normalizes this format)
...
- If `notarytool` rejects `--issuer`, the build now retries automatically without it so individual API keys keep working.
